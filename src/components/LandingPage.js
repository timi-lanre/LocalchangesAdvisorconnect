...
const LandingPage = () => {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (event === 'SIGNED_IN' && session) {
        ...
        // Only redirect when user actively signs in, not on page load
  ...
  const checkUser = async () => {
    try {
      ...
      setUser(user);
      // Remove auto-redirect - let users stay on landing page
      // This allows both new and existing users to see the landing page
    } catch (error) {
  ...
  const handleSignIn = () => {
    if (user) {
      ...
      // If not logged in, open signin modal
      setAuthMode('signin');
      setAuthModalOpen(true);
    }
  };
