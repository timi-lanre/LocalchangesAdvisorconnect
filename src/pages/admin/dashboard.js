...
const AdminDashboard = () => {
  ...
  // Fixed navigation function - removed the alerts and just navigate directly
  const handleNavigation = async (path, cardTitle) => {
    console.log(`🔄 Navigation: ${cardTitle} -> ${path}`);
    ...
    try {
      ...
      // Only show error if navigation actually fails, not preemptively
      setError(`Failed to navigate to ${cardTitle}. Please try again.`);
    } finally {
      setNavigating('');
    }
  };
