// src/pages/admin/dashboard.js - Fixed with working navigation
...
const AdminDashboard = () => {
  ...
  try {
    ...
    if (!isAdminUser) {
      console.error('❌ Admin check failed - redirecting to login');
    ...
    // Try to load stats with better error handling
    try {
      console.log('📊 Loading user stats...');
      const userStats = await authService.getUserStats();
      console.log('✅ User stats loaded:', userStats);
      
      console.log('📊 Loading advisor stats...');
      const advisorStats = await authService.getAdvisorStats();
      console.log('✅ Advisor stats loaded:', advisorStats);
      
      setStats({
        ...userStats,
        totalAdvisors: advisorStats.totalAdvisors || 0
      });
    } catch (statsError) {
      console.error('❌ Stats loading failed:', statsError);
      setError('Failed to load statistics: ' + statsError.message);
  ...
  // Admin cards with correct paths
  const adminCards = [
    {
      id: 'users',
      ...
      <Container maxWidth="xl" sx={{ py: 4 }}>
        {error && (
          <Alert 
            ...
            action={
              <Button size="small" onClick={() => setError(null)} sx={{ color: 'white' }}>
                Dismiss
              </Button>
            }
          >
